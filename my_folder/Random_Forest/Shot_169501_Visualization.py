...
def create_comprehensive_visualization(shot_data):
    ...
    fig.suptitle(f'Shot {shot_data["shot"].iloc[0]} Analysis: Random Forest Predictions vs Actual States', 
    ...
    ax3.set_title('fs04 Time Series with Prediction Accuracy Overlay (ENLARGED)', fontsize=18, fontweight='bold')
    ...
    # Adjust layout to prevent overlap - give more space for legends
    plt.tight_layout()
    plt.subplots_adjust(right=0.88, hspace=0.3)  # More space for legends and between subplots
...
def create_detailed_analysis_plot(shot_data):
    ...
    fig.suptitle(f'Shot {shot_data["shot"].iloc[0]} Detailed Analysis', fontsize=20, fontweight='bold')
...
def main():
    ...
    # Create prediction_visualizations directory if it doesn't exist
    output_dir = 'prediction_visualizations'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        print(f"Created directory: {output_dir}")
    
    # Save plots in the prediction_visualizations folder
