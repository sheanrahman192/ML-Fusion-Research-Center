...
#EVAL VARIANT: Uses combined_database.csv with a fixed list of 42 held-out test shots.
...
# ==== EVAL CONFIG ====
DATA_CSV_PATH = '/mnt/homes/sr4240/my_folder/combined_database.csv'
TEST_SHOTS = [
    169449, 169457, 169460, 169463, 169467, 169470, 169473, 169476, 169479, 169500,
    169503, 169506, 169959, 169966, 170063, 170070, 170090, 170114, 175673, 175682,
    175686, 175695, 175701, 179350, 179363, 183153, 183185, 190276, 190284, 190733,
    191662, 191665, 191674, 191683, 191686, 191689, 191975, 191978, 191981, 191984,
    191987, 191990,
]
VAL_FRACTION_OF_TRAIN = 0.15  # carve a deterministic val split from train shots
MODEL_NAME = 'PatchTST'
...
def load_and_prepare_data():
    """Load and preprocess the combined plasma database for the fixed-shot eval split.

    Notes vs. the original script:
      - Reads combined_database.csv (priority union of plasma_data.csv and the
        label-propagated DB).
      - Combined DB labels states as {-1, 0, 1, 2, 3}; -1 is invalid and dropped,
        0 = Suppressed, 1/2/3 = Dithering/ELMing/Mitigated. The binary mapping is
        applied later in window creation; here we only filter to valid labels.
      - Scaler is fit on the TRAIN shots only (no leakage from held-out test shots).
    """
    print("Loading data...")
    df = pd.read_csv(DATA_CSV_PATH)

    # Original script removed shot 191675; keep that exclusion (not in the test list anyway)
    ...
    # Calculate fs04 rate of change (per-shot) -- preserved from original
    ...
    X = df_sorted[selected_features].values.astype(np.float64)
    y = df_sorted['state'].values
    times = df_sorted['time'].values
    shots = df_sorted['shot'].values

    valid_mask = ~np.isnan(X).any(axis=1) & ~pd.isna(y) & ~np.isnan(times)
    X = X[valid_mask]
    y = y[valid_mask]
    times = times[valid_mask]
    shots = shots[valid_mask]

    # Drop -1 (invalid label) rows; keep only {0,1,2,3}
    y_int = y.astype(int)
    keep_label = np.isin(y_int, [0, 1, 2, 3])
    X = X[keep_label]
    y = y_int[keep_label]
    times = times[keep_label]
    shots = shots[keep_label]

    print(f"Data shape after cleaning: {X.shape}")
    print(f"Label distribution: {Counter(y)}")
    print(f"Unique shots after cleaning: {len(np.unique(shots))}")

    # Fit scaler on TRAIN shots only to avoid leakage from held-out test shots
    test_mask = np.isin(shots, list(TEST_SHOTS))
...
def create_windows_with_fixed_shot_split(X, y, times, shots, window_size=WINDOW_SIZE, prediction_horizon_ms=50):
    """Create windows using a FIXED held-out test shot list.

    - The 42 shots in TEST_SHOTS form the held-out test set.
    - All other shots in the CSV are TRAIN shots.
    - A deterministic VAL_FRACTION_OF_TRAIN slice of the train shots is set aside
      as a validation set for early stopping and threshold selection
      (no test leakage; val shots are disjoint from test shots).
    - Combined-DB binary mapping: 0 -> 0 (Suppressed), {1,2,3} -> 1 (D/E/M).
    """
    print(f"Creating windows of size {window_size} (predicting {prediction_horizon_ms}ms in the future)...")
    print("Splitting by SHOT NUMBER using a FIXED test-shot list")
    print("Binary classification: Suppressed (0) vs Dithering/ELMing/Mitigated (1)")

    unique_shots = np.unique(shots)
    n_shots = len(unique_shots)
    print(f"Total unique shots after cleaning: {n_shots}")

    test_shot_set = set(int(s) for s in TEST_SHOTS)
    test_shots_present = sorted(s for s in unique_shots if int(s) in test_shot_set)
    train_pool_shots = sorted(s for s in unique_shots if int(s) not in test_shot_set)
    test_shots_missing = sorted(test_shot_set - set(int(s) for s in unique_shots))

    print(f"Test shots present (have any valid feature rows): {len(test_shots_present)} / {len(test_shot_set)}")
    if test_shots_missing:
        print(f"Test shots with NO valid feature rows (will yield 0 windows): {test_shots_missing}")

    # Deterministic val carve-out from the train pool
    rng = np.random.RandomState(48)
    train_pool_arr = np.array(train_pool_shots)
    rng.shuffle(train_pool_arr)
    n_val = max(1, int(round(len(train_pool_arr) * VAL_FRACTION_OF_TRAIN)))
    val_shots = set(int(s) for s in train_pool_arr[:n_val])
    train_shots = set(int(s) for s in train_pool_arr[n_val:])
    test_shots_set = set(int(s) for s in test_shots_present)

    print(f"Shot split: Train={len(train_shots)}, Val={len(val_shots)}, Test={len(test_shots_set)} "
          f"(test list size={len(test_shot_set)}, of which present={len(test_shots_present)})")
    ...
    val_current_states = []
    test_current_states = []
    test_window_shots = []  # per-test-window shot id, for per-shot eval

    # combined_database.csv encodes states as {0,1,2,3}
    binary_label_mapping = {0: 0, 1: 1, 2: 1, 3: 1}

    windows_created = 0
    windows_skipped_no_future = 0
    windows_skipped_invalid_label = 0
    per_shot_window_count = {}
    ...
    for shot_id in unique_shots:
        ...
        if len(shot_indices) < window_size:
            per_shot_window_count[int(shot_id)] = 0
            continue
        ...
        for i in range(len(shot_indices) - window_size + 1):
            ...
            if not np.isnan(window).any() and not np.isinf(window).any():
                ...
                if target_shot_list is not None:
                    target_shot_list.append(sid_int)
                windows_created += 1
                shot_added += 1

        per_shot_window_count[sid_int] = shot_added
    ...
    test_window_shots = np.array(test_window_shots)
    ...
    print(f"  Test: {len(test_windows)} windows from {len(test_shots_set)} shots (out of {len(test_shot_set)} listed)")
    ...
    split_meta = {
        ...
        'val_shots': sorted(val_shots),
        'test_shots_present': sorted(test_shots_set),
        'test_shots_missing': test_shots_missing,
        'per_shot_window_count': per_shot_window_count,
        'test_window_shots': test_window_shots,
    }
...
def main():
    """Main training pipeline -- fixed-shot evaluation variant."""
    print("=" * 60)
    print("PatchTST EVAL on combined_database.csv (fixed test shots)")
    print("=" * 60)
    ...
    print(f"Split: FIXED test shots ({len(TEST_SHOTS)}); train = all other shots in CSV")
    print("=" * 60)
    ...
    (train_X, train_y, train_current_states,
     ...
     split_meta) = create_windows_with_fixed_shot_split(
    ...
    if len(test_X) == 0:
        raise RuntimeError("No test windows produced; check test shot list and feature availability.")
    ...
    with torch.no_grad():
        _ = model(test_batch)
    forward_time = _time.time() - fwd_start
    print(f"Forward pass time for batch of {test_batch.shape[0]}: {forward_time:.3f} seconds")

    n_epochs = int(os.environ.get('PATCHTST_EVAL_EPOCHS', '40'))
    print(f"\nStarting training for up to {n_epochs} epochs (early stopping enabled)...")
    train_start = _time.time()
    train_losses, val_losses, train_accs, val_accs = train_model(
        model, train_loader, val_loader, device, class_weights_tensor, n_epochs=n_epochs
    )
    train_time = _time.time() - train_start
    print(f"\nWall-clock training time: {train_time:.2f}s ({train_time/60:.2f} min) over {len(train_losses)} epoch(s)")
    ...
    print(f"\nWall-clock inference time over test set ({len(all_labels)} samples): {inf_time:.3f}s")
    ...
    try:
        ...
        print(f"plot_results failed (non-fatal): {e}")

    # ---- Final structured metric report ----
    ...
    macro_f = f1_score(all_labels, all_preds, average='macro', zero_division=0)
    per_class_p = precision_score(all_labels, all_preds, average=None, labels=[0, 1], zero_division=0)
    per_class_r = recall_score(all_labels, all_preds, average=None, labels=[0, 1], zero_division=0)
    per_class_f = f1_score(all_labels, all_preds, average=None, labels=[0, 1], zero_division=0)
    ...
    test_window_shots = split_meta['test_window_shots']
    per_shot_rows = []
    for sid in split_meta['test_shots_present'] + split_meta['test_shots_missing']:
        mask = test_window_shots == sid
        n = int(mask.sum())
        if n > 0:
            acc = accuracy_score(all_labels[mask], all_preds[mask])
            n_pos = int(np.sum(all_labels[mask] == 1))
            n_neg = int(np.sum(all_labels[mask] == 0))
        else:
            acc = float('nan')
            n_pos = 0
            n_neg = 0
        per_shot_rows.append((sid, n, n_neg, n_pos, acc))

    print("\n" + "=" * 72)
    print(f"FINAL STRUCTURED REPORT  |  model={MODEL_NAME}  horizon={PREDICTION_HORIZON_MS}ms")
    print("=" * 72)
    print(f"data_csv: {DATA_CSV_PATH}")
    print(f"train_shots_count: {len(split_meta['train_shots'])}")
    print(f"val_shots_count:   {len(split_meta['val_shots'])}")
    print(f"test_shots_listed: {len(TEST_SHOTS)}")
    print(f"test_shots_with_data: {len(split_meta['test_shots_present'])}")
    print(f"test_shots_missing:   {len(split_meta['test_shots_missing'])}  -> {split_meta['test_shots_missing']}")
    print(f"train_samples: {len(train_X)}")
    print(f"val_samples:   {len(val_X)}")
    print(f"test_samples:  {len(test_X)}")
    print(f"epochs_run: {len(train_losses)}  (max_requested={n_epochs})")
    print(f"train_time_seconds: {train_time:.2f}")
    print(f"inference_time_seconds: {inf_time:.3f}")
    print(f"optimal_threshold: {optimal_threshold:.4f}")
    print()
    print("==CSV-METRICS==  (model,metric,value)")
    print(f"{MODEL_NAME},accuracy,{test_acc:.6f}")
    print(f"{MODEL_NAME},precision_macro,{macro_p:.6f}")
    print(f"{MODEL_NAME},recall_macro,{macro_r:.6f}")
    print(f"{MODEL_NAME},f1_macro,{macro_f:.6f}")
    print(f"{MODEL_NAME},precision_class0,{per_class_p[0]:.6f}")
    print(f"{MODEL_NAME},precision_class1,{per_class_p[1]:.6f}")
    print(f"{MODEL_NAME},recall_class0,{per_class_r[0]:.6f}")
    print(f"{MODEL_NAME},recall_class1,{per_class_r[1]:.6f}")
    print(f"{MODEL_NAME},f1_class0,{per_class_f[0]:.6f}")
    print(f"{MODEL_NAME},f1_class1,{per_class_f[1]:.6f}")
    print(f"{MODEL_NAME},roc_auc,{roc_auc:.6f}")
    print(f"{MODEL_NAME},train_seconds,{train_time:.2f}")
    print(f"{MODEL_NAME},inference_seconds,{inf_time:.3f}")
    print(f"{MODEL_NAME},train_samples,{len(train_X)}")
    print(f"{MODEL_NAME},test_samples,{len(test_X)}")
    print(f"{MODEL_NAME},train_shots,{len(split_meta['train_shots'])}")
    print(f"{MODEL_NAME},test_shots_with_data,{len(split_meta['test_shots_present'])}")
    print()
    print("Confusion matrix (rows=true, cols=pred; classes=[Suppressed, D/E/M]):")
    print(cm)
    print()
    print(f"Per-class metrics:")
    for c, name in enumerate(class_names):
        print(f"  class {c} ({name}): precision={per_class_p[c]:.4f}  recall={per_class_r[c]:.4f}  f1={per_class_f[c]:.4f}")
    print(f"Macro: precision={macro_p:.4f}  recall={macro_r:.4f}  f1={macro_f:.4f}")
    print(f"Accuracy: {test_acc:.4f}")
    print(f"ROC-AUC: {roc_auc:.4f}")
    print()
    print("Per-shot accuracy table:")
    print(f"  {'shot':>8}  {'n_windows':>10}  {'n_class0':>9}  {'n_class1':>9}  {'accuracy':>9}")
    for sid, n, n0, n1, acc in per_shot_rows:
        acc_str = f"{acc:.4f}" if not np.isnan(acc) else "  n/a "
        print(f"  {sid:>8d}  {n:>10d}  {n0:>9d}  {n1:>9d}  {acc_str:>9}")

    # Save a CSV alongside results for downstream cross-model comparison
    out_csv = f'patchtst_{PREDICTION_HORIZON_MS}ms_eval_metrics{VARIANT_SUFFIX}.csv'
    rows = [
        (MODEL_NAME, 'accuracy', f'{test_acc:.6f}'),
        (MODEL_NAME, 'precision_macro', f'{macro_p:.6f}'),
        (MODEL_NAME, 'recall_macro', f'{macro_r:.6f}'),
        (MODEL_NAME, 'f1_macro', f'{macro_f:.6f}'),
        (MODEL_NAME, 'precision_class0', f'{per_class_p[0]:.6f}'),
        (MODEL_NAME, 'precision_class1', f'{per_class_p[1]:.6f}'),
        (MODEL_NAME, 'recall_class0', f'{per_class_r[0]:.6f}'),
        (MODEL_NAME, 'recall_class1', f'{per_class_r[1]:.6f}'),
        (MODEL_NAME, 'f1_class0', f'{per_class_f[0]:.6f}'),
        (MODEL_NAME, 'f1_class1', f'{per_class_f[1]:.6f}'),
        (MODEL_NAME, 'roc_auc', f'{roc_auc:.6f}'),
        (MODEL_NAME, 'train_seconds', f'{train_time:.2f}'),
        (MODEL_NAME, 'inference_seconds', f'{inf_time:.3f}'),
        (MODEL_NAME, 'train_samples', str(len(train_X))),
        (MODEL_NAME, 'test_samples', str(len(test_X))),
        (MODEL_NAME, 'train_shots', str(len(split_meta['train_shots']))),
        (MODEL_NAME, 'test_shots_with_data', str(len(split_meta['test_shots_present']))),
    ]
    try:
        with open(out_csv, 'w') as f:
            ...
            for r in rows:
                f.write(','.join(r) + '\n')
        print(f"\nSaved metric CSV: {out_csv}")
    except Exception as e:
        print(f"Failed to write metric CSV: {e}")

    print("\n" + "=" * 60)
    print(f"Eval Complete! (Predicting {PREDICTION_HORIZON_MS}ms into the future - Binary - PatchTST)")
    print("=" * 60)
