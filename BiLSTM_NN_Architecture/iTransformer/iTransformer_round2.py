...
class iTransformer(nn.Module):
    ...
    def __init__(self, n_features, seq_len=150, n_classes=4,
        ...
        # ROUND 2: mean-pool variate tokens -> small head.
        # The old flatten(6*d_model)->256->128 head was ~36% of params and
        # the main overfitting source. Pooling is permutation-invariant
        # (correct for unordered variate tokens) and far smaller.
    ...
    def forward(self, x):
        ...
        # ROUND 2: mean-pool over variate tokens -> (batch, d_model)
...
def train_model(model, train_loader, val_loader, device, n_epochs=50, class_weights=None):
    ...
    # ROUND 1/2: AdamW; ROUND 2 lowers LR (1e-3->5e-4) and raises weight
    # decay (1e-2->5e-2) to curb the fast overfitting seen in round 1.
    optimizer = optim.AdamW(model.parameters(), lr=5e-4, weight_decay=5e-2)
    # ROUND 2: linear warmup (5 ep) + cosine decay — standard for
    # transformers, replaces plateau scheduling which overfit early.
    warmup_epochs = 5
    def lr_lambda(epoch):
...
def main():
    ...
    n_layers = 3
    d_ff = 256
    dropout = 0.3  # ROUND 2: raised from 0.2 to curb overfitting
