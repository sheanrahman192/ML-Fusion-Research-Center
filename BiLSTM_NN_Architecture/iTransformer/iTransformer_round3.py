...
class iTransformer(nn.Module):
    ...
    def __init__(self, n_features, seq_len=150, n_classes=4,
        ...
        # ROUND 3: flatten head restored (R2 mean-pool underfit by
        # discarding per-variate structure across the 6 tokens).
...
def train_model(model, train_loader, val_loader, device, n_epochs=50, class_weights=None):
    ...
    # ROUND 3: revert LR/WD to R1's winning values (R2's lr=5e-4 +
    # wd=5e-2 underfit). Keep AdamW.
