...
def train_model(model, train_loader, val_loader, device, n_epochs=50):
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_lstm_50ms_random_shot.pth')
            patience_counter = 0
...
def plot_results(train_losses, val_losses, train_accs, val_accs, all_preds, all_labels, class_names):
    ...
    plt.tight_layout()
    plt.savefig(f'lstm_{PREDICTION_HORIZON_MS}ms_random_shot_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Results saved to 'lstm_{PREDICTION_HORIZON_MS}ms_random_shot_results.png'")
...
def main():
    ...
    print("=" * 60)
    print("LSTM-NN Model for Plasma Classification")
    print("=" * 60)
    ...
    # Load best model
    print("\nLoading best model...")
    model.load_state_dict(torch.load('best_lstm_50ms_random_shot.pth'))
    ...
    print("\n" + "=" * 60)
    print(f"Training Complete! (Predicting {PREDICTION_HORIZON_MS}ms into the future)")
    print("=" * 60)
